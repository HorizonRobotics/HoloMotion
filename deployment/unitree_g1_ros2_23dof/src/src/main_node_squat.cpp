...
public:
  humanoid_controller() : Node("humanoid_controller") {
    RCLCPP_INFO(this->get_logger(), "Using main_node_stand !!!");
    ...
    motion_type_subscriber_ =
        this->create_subscription<std_msgs::msg::String>(
            "/motion_type", 10,
            std::bind(&humanoid_controller::MotionTypeHandler, this, _1));
    ...
    time_ = 0;
    duration_ = 3; // 3 s
    current_motion_type_ = "other_motion"; // Initialize with default value
  }
...
private:
  ...
  // Helper to identify wrist joints that should be disabled (mode = 0)
  bool isWristJoint(const std::string &dof_name) const {
    return dof_name == "right_wrist_roll_joint" ||
           dof_name == "right_wrist_pitch_joint" ||
           dof_name == "right_wrist_yaw_joint" ||
           dof_name == "left_wrist_roll_joint" ||
           dof_name == "left_wrist_pitch_joint" ||
           dof_name == "left_wrist_yaw_joint";
  }
  ...
  void loadConfig(const std::string &config_path) {
    try {
      ...
      // Load default angles
      auto angles = config["default_joint_angles_squat"];
  ...
  void SendDefaultPositionCommand() {
    ...
    for (int i = 0; i < G1_NUM_MOTOR; ++i) {
      std::string dof_name = complete_dof_order[i];
      if (isWristJoint(dof_name)) {
        // Disable wrist joints
        low_command.motor_cmd[i].mode = 0;
        low_command.motor_cmd[i].tau = 0.0;
        low_command.motor_cmd[i].q = 0.0;
        low_command.motor_cmd[i].dq = 0.0;
        low_command.motor_cmd[i].kp = 0.0;
  ...
  void SendPolicyCommand() {
    ...
    // Check if current motion is motion_file or motion_file_3
    bool should_disable_wrist = (current_motion_type_ == "motion_file" || current_motion_type_ == "motion_file_3");
    ...
    for (const auto &pair : target_dof_pos) {
      ...
      // Disable wrist joints if motion is motion_file or motion_file_3
      if (should_disable_wrist && isWristJoint(dof_name)) {
        low_command.motor_cmd[motor_idx].mode = 0;
        low_command.motor_cmd[motor_idx].tau = 0.0;
        low_command.motor_cmd[motor_idx].q = 0.0;
        low_command.motor_cmd[motor_idx].dq = 0.0;
        low_command.motor_cmd[motor_idx].kp = 0.0;
        low_command.motor_cmd[motor_idx].kd = 0.0;
        continue;
      }
  ...
  void MotionTypeHandler(
      const std_msgs::msg::String::SharedPtr message) {
    current_motion_type_ = message->data;
    RCLCPP_INFO(this->get_logger(), "Received motion type: %s",
                current_motion_type_.c_str());
  }
  ...
  std::string current_motion_type_;
