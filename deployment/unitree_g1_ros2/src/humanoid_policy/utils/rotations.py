...
#
# Note:
#   Some utility functions in this file may be inspired by or adapted from
#   various open-source projects, including but not limited to PyTorch,
#   IsaacGym, and other robotics/math libraries.
#   If you are an original author and your contribution is not properly
#   credited, please contact us for proper attribution.
...
def quat_angle_axis(x: Tensor, w_last: bool) -> Tuple[Tensor, Tensor]:
    ...
    The axis is normalized to unit length. The angle is guaranteed to be
    between [0, pi].
    """
    if w_last:
        w = x[..., -1]
...
def quat_normalize(q):
    """Return normalized 3D rotation quaternion."""
...
def quat_mul_norm(x, y, w_last):
    ...
    r"""Combine two set of 3D rotations together using *** operator.

    The shape needs to be broadcastable.
    """
...
def transform_from_rotation_translation(
    ...
    """Construct a transform from a quaternion and 3D translation."""
...
def matrix_to_quaternion(matrix: torch.Tensor) -> torch.Tensor:
    ...
    Reference:
        Inspired by PyTorch3D and other open-source robotics libraries.
    """
    ...
    # Pick best-conditioned candidate if not for numerical problems.
