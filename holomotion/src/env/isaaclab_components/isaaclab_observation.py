...
class ObservationFunctions:
    ...
    def _get_obs_head_pos_quat_vel(
        ...
        try:
            ...
            raise ValueError(f"'torso_link' not found in body_names: {body_names}")
        ...
        pitch = torch.tensor(0.04014257279586953, dtype=torch.float, device=device)
        half = pitch * 0.5
        # WXYZ
        ...
        if not hasattr(env, "head_anchor_set"):
            env.head_anchor_set = torch.zeros(B, dtype=torch.bool, device=device)
        ...
        pos_rel = isaaclab_math.quat_apply(q0_inv, pos_torso - env.head_anchor_pos)
    ...
    def _get_obs_global_robot_bodylink_pos(
        ...
        robot_ptr = env.scene[robot_asset_name]
        keybody_idxs = ObservationFunctions._get_body_indices(robot_ptr, keybody_names)
    ...
    def _get_obs_root_rel_robot_bodylink_rot_xyzw(
        ...
        return ObservationFunctions._get_obs_root_rel_robot_bodylink_rot_wxyz(
            env, robot_asset_name, keybody_names
        )[..., [1, 2, 3, 0]]  # [num_envs, num_keybodies, 4] - convert WXYZ to XYZW
    ...
    def _get_obs_root_rel_robot_bodylink_lin_vel(
        ...
        rel_lin_vel_w = keybody_global_lin_vel - root_global_lin_vel.unsqueeze(1)
    ...
    def _get_obs_root_rel_robot_bodylink_ang_vel(
        ...
        rel_ang_vel_w = keybody_global_ang_vel - root_global_ang_vel.unsqueeze(1)
    ...
    def _get_obs_global_robot_bodylink_rot_wxyz_flat(
        ...
        bodylink_rot = ObservationFunctions._get_obs_global_robot_bodylink_rot_wxyz(
            env, robot_asset_name, keybody_names
        )  # [num_envs, num_keybodies, 4]
    ...
    def _get_obs_global_robot_bodylink_rot_xyzw_flat(
        ...
        bodylink_rot = ObservationFunctions._get_obs_global_robot_bodylink_rot_xyzw(
            env, robot_asset_name, keybody_names
        )  # [num_envs, num_keybodies, 4]
        return bodylink_rot.reshape(
            bodylink_rot.shape[0], -1
    ...
    def _get_obs_global_robot_bodylink_rot_mat_flat(
        ...
        bodylink_rot_mat = ObservationFunctions._get_obs_global_robot_bodylink_rot_mat(
            env, robot_asset_name, keybody_names
        )  # [num_envs, num_keybodies, 6]
    ...
    def _get_obs_global_robot_bodylink_lin_vel_flat(
        ...
        bodylink_lin_vel = ObservationFunctions._get_obs_global_robot_bodylink_lin_vel(
            env, robot_asset_name, keybody_names
        )  # [num_envs, num_keybodies, 3]
    ...
    def _get_obs_global_robot_bodylink_ang_vel_flat(
        ...
        bodylink_ang_vel = ObservationFunctions._get_obs_global_robot_bodylink_ang_vel(
            env, robot_asset_name, keybody_names
        )  # [num_envs, num_keybodies, 3]
    ...
    def _get_obs_root_rel_robot_bodylink_pos_flat(
        ...
        bodylink_pos = ObservationFunctions._get_obs_root_rel_robot_bodylink_pos(
            env, robot_asset_name, keybody_names
        )  # [num_envs, num_keybodies, 3]
        return bodylink_pos.reshape(
            bodylink_pos.shape[0], -1
    ...
    def _get_obs_root_rel_robot_bodylink_rot_wxyz_flat(
        ...
        bodylink_rot = ObservationFunctions._get_obs_root_rel_robot_bodylink_rot_wxyz(
            env, robot_asset_name, keybody_names
        )  # [num_envs, num_keybodies, 4]
        return bodylink_rot.reshape(
            bodylink_rot.shape[0], -1
    ...
    def _get_obs_root_rel_robot_bodylink_rot_xyzw_flat(
        ...
        bodylink_rot = ObservationFunctions._get_obs_root_rel_robot_bodylink_rot_xyzw(
            env, robot_asset_name, keybody_names
        )  # [num_envs, num_keybodies, 4]
        return bodylink_rot.reshape(
            bodylink_rot.shape[0], -1
    ...
    def _get_obs_global_anchor_diff(
        ...
        command = env.command_manager.get_term(ref_motion_command_name)
        global_ref_motion_anchor_pos = command.ref_motion_anchor_bodylink_global_pos_cur
        ...
        for obs_term_dict in group_cfg["atomic_obs_list"]:
            for obs_name, obs_params in obs_term_dict.items():
                ...
                if hasattr(ObservationFunctions, method_name):
                    ...
                    raise ValueError(f"Unknown observation function: {obs_name}")
